...
def set_idle_photo(image_url: str) -> bool:
    """
    Sets OBS to show a static photo (idle state).
    No lip movement — avatar is just listening.
    """
    global _obs_client
    ...
    try:
        # HTML page showing a static photo
        html_content = f"""
        <!DOCTYPE html>
        <html>
        <head>
        ...
        page_url = f"{FLASK_URL}/idle-player?t={int(time.time())}"
